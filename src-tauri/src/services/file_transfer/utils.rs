...
pub fn update_activity(app_handle: &AppHandle) {
    if let Ok(mut guard) = app_handle.state::<ServerActivityState>().last_activity.lock() {
...
pub fn get_app_logo_base64(_app: &AppHandle) -> String {
    ...
    format!("data:image/png;base64,{}", general_purpose::STANDARD.encode(ICON_BYTES))
}

pub fn score_interface(name: &str, ip: &str) -> i32 {
    let mut score = 0;
    if name.contains("wi-fi") || name.contains("wlan") { score += 10; }
    if name.contains("ethernet") { score += 5; }
    if ip.starts_with("192.168.") { score += 3; }
    if ip.starts_with("10.") { score += 2; }
    score
}
